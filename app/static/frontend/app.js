...
async function generatePoCFromAPI(cveId) {
    ...
    try {
        ...
        if (pocCode && data.code) {
            ...
            pocCode.textContent = `#!/usr/bin/env python3
"""
...
if __name__ == "__main__":
    ...
    exploit_vulnerability(target)`;
        ...
        const pocDocs = document.getElementById('poc-docs-tab');
        if (pocDocs && data.documentation) {
            let docsHtml = '<div class="space-y-4">';
            
            if (data.documentation.description) {
                docsHtml += `
                    <div>
                        <h6 class="font-medium text-gray-900 mb-2">Description</h6>
                        <p class="text-gray-700">${escapeHtml(data.documentation.description)}</p>
                    </div>
                `;
            }
            
            if (data.documentation.usage) {
                docsHtml += `
                    <div>
                        <h6 class="font-medium text-gray-900 mb-2">Usage Instructions</h6>
                        <p class="text-gray-700">${escapeHtml(data.documentation.usage)}</p>
                    </div>
                `;
            }
            
            if (data.documentation.warnings) {
                docsHtml += `
                    <div>
                        <h6 class="font-medium text-gray-900 mb-2">⚠️ Warnings</h6>
                        <p class="text-red-600">${escapeHtml(data.documentation.warnings)}</p>
                    </div>
                `;
            }
            
            docsHtml += '</div>';
            pocDocs.innerHTML = docsHtml;
        }
        
        // Display metadata if available
        if (data.metadata) {
            const pocComplexity = document.getElementById('poc-complexity');
            if (pocComplexity && data.metadata.complexity) {
                pocComplexity.innerHTML = `<i class="fas fa-brain mr-1"></i>Complexity: ${data.metadata.complexity}`;
            }
        }
...
async function applyAdvancedFilters() {
    ...
    try {
        ...
        const filterCount = Object.keys(searchParams).filter(key => 
            !['limit', 'sort_by', 'sort_order', 'search_type', 'include_descriptions', 'include_details', 'query'].includes(key) && 
